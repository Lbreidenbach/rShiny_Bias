...
ci_ridges = function(run, title =NULL, subtitle=NULL){
  ...
  L1 = grid::rectGrob(height = 0.6, width = 0.8, y=.41,gp = gpar(fill = "lightblue2", col = "black", lwd = 2))
  L2 = grid::rectGrob(height = 0.6, width = 0.8,y=.41, gp = gpar(fill = "grey50",  col = "black", lwd = 2))
  L3 = grid::linesGrob(x = unit(c(0.05, 0.9), "npc"), y = unit(c(.3, .3), "npc"), gp = gpar( col = "navy", lwd = 3.8, lty = "dashed"))
  T1 = grid::textGrob("= Yes", x = 0, just = "left",y=.4,gp = gpar(fontsize=14))
  T2 = grid::textGrob("= No", x = 0, just = "left",y=.4, gp = gpar(fontsize=14))
  T3 = grid::textGrob("Set true effect size =", x = .36,y=.3, just = "left",gp = gpar(fontsize=14, fontface = "bold"))
  
  
  # Construct a gtable - 2 columns X 4 rows
  leg = gtable::gtable(width = unit(c(8,1.5,1.5,1.5,1.5), "cm"), height = unit(c(2,1), "cm"))
...
error_catch = function(dag_string){
  tryCatch({dag_string}, {
    if(substring(dag_string, 1,1) != "~"){
      stop("Remember to start each formula with a '~'")
    } 
    dag = HydeNetwork(eval(str2lang(paste0(dag_string))))
    if(length(try(plot( dag ), silent = T))== 1){
...
get_nodes = function(dag_string){
  ...
  if(class(tryCatch( error_catch(dag_string), error =  function(x) x=1)) =="HydeNetwork"){
    node_test = error_catch(dag_string)[["nodes"]]
    return(node_test)
